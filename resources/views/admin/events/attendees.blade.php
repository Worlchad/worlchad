                            ...
                            <div class="table-responsive">
                                <table id="myTable" class="table table-striped">
                                    ...
                                    @forelse($event->attendees as $attendee)
                                        <tr>
                                        <td><img class="img-circle" src="{{asset('uploads/events/attendees/'.$attendee->image)}}" width="50" alt=""> </td>
                                            <td>{{$attendee->first_name}}</td>
                                            <td>{{$attendee->last_name}}</td>
                                            <td>{{$attendee->email}}</td>
                                            <td>{{$attendee->phone}}</td>
                                            <td>{{$attendee->gender =='m'?'Male':'Female'}}</td>
                                            <td>{{$attendee->country->name}}</td>
                                            <td>{{$attendee->state->name}}</td>
                                            <td>{{$attendee->status}}</td>
                                            <td>{{$attendee->reference_id}}</td>
                                            <td>

                                            </td>
                                        </tr>
                                    @empty
                                        <h3>No Attendee yet</h3>
                                    @endforelse
                                    </tbody>
                                </table>
                            </div>
